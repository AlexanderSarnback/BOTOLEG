...
{
    /// <summary>
    /// The <see cref="UrlScanCommandHandler"/> registers a pattern with the <see cref="ITeamsCommandHandler"/> and 
    /// responds with an Adaptive Card if the user types the <see cref="TriggerPatterns"/>.
    /// </summary>
    public class UrlScanCommandHandler : ITeamsCommandHandler
    {
        private readonly ILogger<PingScanCommandHandler> _logger;
        private readonly string _adaptiveCardFilePath = Path.Combine(".", "Resources", "UrlScanCard.json");

        public IEnumerable<ITriggerPattern> TriggerPatterns => new List<ITriggerPattern>
        {
            // Used to trigger the command handler if the command text contains 'ping'
            new RegExpTrigger("Url")
        };

        public UrlScanCommandHandler(ILogger<PingScanCommandHandler> logger)
        {
            _logger = logger;
        }
        ...
        public async Task<ICommandResponse> HandleCommandAsync(ITurnContext turnContext, CommandMessage message, CancellationToken cancellationToken = default)
        {
            ...
            (
                ...
                {
                    Url = "Scan your URL ",
                    UrlBody = "THIS SPACE IS FOR THE DATA FROM THE URL SCAN",
                }
            );
